...
ShowCloneDialog:
    ...
    Gui Add, Picture, x8 y8 w32 h32, Icons\WCT.ico
    Gui Font, s12 c0x003399, Segoe UI
    Gui Add, Text, x52 y10 w421 h23 +0x200 BackgroundTrans, Activate the target window and click the "Clone" button (F8)
    Gui Font
    Gui Font,, MS Shell Dlg 2
    Gui Add, ListView, x10 y57 w374 h242 -Hdr, Handle|Title
    LV_ModifyCol(1, 0)
    LV_ModifyCol(2, 350)
    ...
    Gui Add, CheckBox, vIncludeStyles x112 y306 w200 h23, Include styles for all controls in output
    Gui Add, StatusBar,, %A_Space%%A_Space%%A_Space%Warning: the script will be generated in the first tab, replacing its contents
    Gui Show, w482 h360, % WCT_Title
    SetIconEx(hCloneDlg, IconLib, 43)
...
CloneWindow:
    ...
    ncLeftWidth := wi.rcClient.left - wi.rcWindow.left
    ncTopHeight := wi.rcClient.top - wi.rcWindow.top
...
;************************************************************************
    ...
    If (ControlList == "") {
        ...
        Msgbox 0x2030, Window Cloning Tool, % GetClassName(hTargetWnd) . ": unable to clone the window."
        ;Return
    }
    ...
    Loop Parse, ControlList, `n
    {
        ...
        Style := ToHex(ControlStyle)

        Gui %Child%: Add, %AhkName%, hWndhWnd x%x% y%y% w%w% h%h% %Options% %Style%, %ControlText%
        ...
        Register(hWnd, AhkName, GetClassNN(hWnd), EscapeChars(ControlText)
        , x, y, w, h, "", "", "", Options, Style, "", "", "", "")
    ...
    Loop % ItemCount {
        ...
        If (ItemID = -1) { ; Submenu
            ...
            If (hSubMenu) {
                ...
                Commands .= "Menu " . MenuName . ", Add, " . OldItemString . ", :"  . Prefix ItemString "Menu`n"
                Continue
            }
        }
        ...
        If (Prefix != "") {
            If (ItemString = "SEPARATOR") {
                Menu, % Prefix, Add
                Commands .= "Menu " . Prefix . ", Add`n"
            } Else {
                Menu, % Prefix, Add, % ItemString, MenuHandler
                ItemString := EscapeChars(ItemString)
                Commands .= "Menu " . prefix . ", Add, " . ItemString . ", MenuHandler`n"
            }
        }
    }
