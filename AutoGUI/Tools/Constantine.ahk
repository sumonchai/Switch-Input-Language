...
Expand    := []
Sorting   := [0, "Sort"]
hCursor   := DllCall("LoadCursor", "UInt", 0, "UInt", 32644)
Global hWindow := 0, hTV := 0, hLV := 0
...
Gui Add, Picture, hWndhSearchPic gSearch x198 y513 w16 h16 Icon10, %IconLib%
Gui Add, Edit, hWndhSearch vKeyword gSearch x218 y512 w200 h21
DllCall("SendMessage", "Ptr", hSearch, "UInt", 0x1501, "Ptr", 1, "WStr", "Search", "Ptr") ; Hint text
...
Args := GetArgs()
Loop % Args.MaxIndex() {
    If (Args[A_Index] != A_ScriptFullPath) {
        GuiControl,, %hSearch%, % Args[A_Index]
    }
}

GuiControl Focus, %hSearch%
...
LoadTreeView(Nodes, TVParentID, XMLPath) {
    ...
    For Node in Nodes {
        ...
        ;OutputDebug % Node.getAttribute("ID") ": " Node.baseName
...
Copy:
    ControlGetFocus Focus, ahk_id %hWindow%
    If (Focus == "Edit1") {
        Send ^C
        Return
    }
...
GoogleSearch:
    ...
    Try {
        ...
        MsgBox 0x2010, Error %A_LastError%, % e.Message . "`n`n" . e.Extra
    }
Return
